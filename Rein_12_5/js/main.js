...
$(function(){
	...
	$(window).scroll(function(){
		...
		if (ScrollTop < 649) {
			var shape_name = $("#one1").data('shape')
			$( ".canvas-wrapper" ).removeClass('moveleft')
			$( ".canvas-wrapper" ).removeClass('showdust')
		}
		if (ScrollTop >= 650 && ScrollTop <= 999) {

			var shape_name = $("#two1").data('shape')
			$( ".canvas-wrapper" ).addClass('moveleft')
			$( ".canvas-wrapper" ).addClass('showdust')
			$( ".seconddiv h1" ).toggleClass('risetext')



		}

		if (ScrollTop >= 1300 && ScrollTop <= 3900) {
			$(".rocket-holder").css("margin-top", ScrollTop*2-1800);
			
		}

		if (ScrollTop >= 1000 && ScrollTop <= 2399) {
var shape_name = $("#three1").data('shape')
$( ".canvas-wrapper" ).removeClass('moveleft')
$( ".canvas-wrapper" ).removeClass('showdust')

		}

		if (ScrollTop >= 2400 && ScrollTop <= 2749) {
var shape_name = $("#four1").data('shape')
$( ".canvas-wrapper" ).removeClass('moveleft')
$( ".canvas-wrapper" ).addClass('showdust')
$( ".alignleft h1" ).addClass('risetext')
		}


		if (ScrollTop >= 2750 && ScrollTop <= 3299) {
		var shape_name = $("#one1").data('shape')
		$( ".alignright h1" ).addClass('risetext')
		$( ".alignleft h1" ).removeClass('risetext')
		}

				if (ScrollTop >= 3300 && ScrollTop <= 3799) {
		var shape_name = $("#five1").data('shape')
		$( ".canvas-wrapper" ).addClass('moveleft')
		$( ".alignright1 h1" ).addClass('risetext')
		$( ".alignright h1" ).removeClass('risetext')
		}

				if (ScrollTop >= 3800 && ScrollTop <= 4099) {
		var shape_name = $("#six1").data('shape')
		$( ".canvas-wrapper" ).removeClass('moveleft')
		$( ".canvas-wrapper" ).removeClass('showdust')
		$( ".alignright1 h1" ).removeClass('risetext')
		}

		if (ScrollTop >= 4100 && ScrollTop <= 4200) {
		var shape_name = $("#six1").data('shape')
		$( ".canvas-wrapper" ).addClass('showdust')
		$( ".seconddiv h1" ).addClass('risetext')
		
		}
		if (ScrollTop >= 4800) {
			 $( "#aura2" ).addClass('dropdown')
			 $( ".seconddiv h1" ).removeClass('risetext')
            $( ".canvas-wrapper" ).removeClass('showdust')
        } else {$( "#aura2" ).removeClass('dropdown')}
...
function initShapes(){
	...
	*/
	...
	var sad_img = $('img#sad');
	shapes.add('sad', new Shape({
		src : sad_img.attr('src'),
		width: sad_img.attr('width'), // x 1.5
		height : sad_img.attr('height'), // x 1.5
		offset : {
			x : (canvas.width - sad_img.attr('width'))/2,
			y : (canvas.height - sad_img.attr('height'))/2
		},
		spread : 20
	}));

	var happy_img = $('img#happy');
	shapes.add('happy', new Shape({
		src : happy_img.attr('src'),
		width: happy_img.attr('width'), // x 1.5
		height : happy_img.attr('height'), // x 1.5
		offset : {
			x : (canvas.width - happy_img.attr('width'))/2,
			y : (canvas.height - happy_img.attr('height'))/2
		},
		spread : 20
	}));
}
